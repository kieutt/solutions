...
**Factor:**
$$\text{factor}(c) = \begin{cases} f(c) - 1 & \text{nếu } p_i = 1 \text{ (gốc)} \\ f(c) & \text{ngược lại} \end{cases}$$

**Số phương án:**
$$N = \prod_{i=0}^{m} H_i$$

**Tổng đỉnh nhánh được thăm:**
...
## 9. Kiến thức cần chuẩn bị

### 9.1. Cấu trúc dữ liệu

- **Danh sách kề (Adjacency List):** Lưu cây hiệu quả
- **Stack:** DFS iterative
...
- **Số học modular:** Phép cộng, nhân mod $10^9+7$
- **Tích tiền tố/hậu tố:** Tính $\prod_{j \neq i}$ trong $O(n)$

### 9.4. Kỹ thuật lập trình

- **Iterative DFS:** Tránh stack overflow với $n = 10^6$
- **Fast I/O:** Đọc input nhanh với $n$ lớn
